...
class Sonny < Player
  ...
  def choose
    if history.empty?
      ...
      choice = history.beat_last_human_move
    end
    self.move = Move.new(choice)
  end
end
...
class Number5 < Player
  ...
  def choose
    if history.empty?
      ...
      choice = history.beat_most_frequent_move
    end
    self.move = Move.new(choice)
  end
end
...
class History
  ...
  def beat_last_human_move
    beat(@log_of_moves.last[:human_move])
  end

  def beat_most_frequent_move
    human_history = @log_of_moves.map { |move| move[:human_move] }
